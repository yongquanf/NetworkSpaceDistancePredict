...
public class Statistics {
     ...
     * edu.NUDT.PDL.cluster.Math_cluster#log2Gamma(double)}), and where
     * <code>&Psi;</code> is the digamma function defined in {@link
     * edu.NUDT.PDL.cluster.Math_cluster#digamma(double)}.
     *
    ...
    public static double klDivergenceDirichlet(double[] xs, double[] ys) {
        ...
        double divergence
            = (logGamma(sumXs)
               - logGamma(sumYs));
        double digammaSumXs = edu.NUDT.PDL.cluster.Math_cluster.digamma(sumXs);
        for (int i = 0; i < xs.length; ++i) {
            divergence +=
                (logGamma(ys[i])
                 - logGamma(xs[i]))
                + (xs[i] - ys[i]) * (edu.NUDT.PDL.cluster.Math_cluster.digamma(xs[i]) - digammaSumXs);
        }
        return divergence;
    }
    ...
    static double logGamma(double x) {
        return edu.NUDT.PDL.cluster.Math_cluster.log2Gamma(x)
            / edu.NUDT.PDL.cluster.Math_cluster.log2(java.lang.Math.E);
    }
    ...
    public static double klDivergence(double[] p, double[] q) {
        ...
        for (int i = 0; i < len; ++i) {
            if (p[i] > 0.0 && p[i] != q[i])
                divergence += p[i] * edu.NUDT.PDL.cluster.Math_cluster.log2(p[i] / q[i]);
        }
        return divergence;
    }
    ...
    public static double[] normalize(double[] probabilityRatios) {
        ...
        double sum = edu.NUDT.PDL.cluster.Math_cluster.sum(probabilityRatios);
        if (sum <= 0.0) {
    ...
    public static double mean(double[] xs) {
        return edu.NUDT.PDL.cluster.Math_cluster.sum(xs) / (double) xs.length;
    }
    ...
    public static double dirichletLog2Prob(double alpha, double[] xs) {
        ...
        int k = xs.length;
        // normalizing term
        double result = edu.NUDT.PDL.cluster.Math_cluster.log2Gamma(k * alpha)
            - k * edu.NUDT.PDL.cluster.Math_cluster.log2Gamma(alpha);
        double alphaMinus1 = alpha - 1;
        for (int i = 0; i < k; ++i)
            result += alphaMinus1 * edu.NUDT.PDL.cluster.Math_cluster.log2(xs[i]);
        return result;
    }
    ...
    public static double dirichletLog2Prob(double[] alphas, double[] xs) {
        ...
        for (int i = 0; i < alphas.length; ++i) {
            alphaSum += alphas[i];
            result -= edu.NUDT.PDL.cluster.Math_cluster.log2Gamma(alphas[i]);
        }
        result += edu.NUDT.PDL.cluster.Math_cluster.log2Gamma(alphaSum);
        for (int i = 0; i < k; ++i)
            result += (alphas[i] - 1) * edu.NUDT.PDL.cluster.Math_cluster.log2(xs[i]);
        return result;
    }
