...
public class Mcsrch
{
    ...
    public static void mcsrch ( int n , double[] x , double f , double[] g , double[] s , int is0 , double[] stp , double ftol , double xtol , int maxfev , int[] info , int[] nfev , double[] wa )
    {
        ...
        while ( true )
        {
            if ( info[0] != -1 )
            {
                ...
                //TODO: evaluate the function value @x
                info[0]=-1;
                return;
            }
    ...
    public static void mcsrch (MFWithGradient FUN, int n , double[] x , double f[] , double[] g , double[] s , int is0 , double[] stp , double ftol , double xtol , int maxfev , int[] info , int[] nfev , double[] wa, double stpmin, double stpmax )
    {
        p5 = 0.5;
        p66 = 0.66;
        xtrapf = 4;
       ...
       // {
        ...
        while ( true )
        {
           // if ( info[0] != -1 )
            ...
            if ( stp[0] == LBFGS.stpmax && f[0] <= ftest1 && dg <= dgtest ) info[0] = 5;

            if ( stp[0] == LBFGS.stpmin && ( f[0] > ftest1 || dg >= dgtest ) ) info[0] = 4;

            if ( nfev[0] >= maxfev ) info[0] = 3;

            if ( brackt[0] && stmax - stmin <= xtol * stmax ) info[0] = 2;

            if ( f[0] <= ftest1 && Math.abs ( dg ) <= LBFGS.gtol * ( - dginit ) ) info[0] = 1;
            ...
            if ( stage1 && f[0] <= ftest1 && dg >= Math.min ( ftol , LBFGS.gtol ) * dginit ) stage1 = false;
            ...
            else
            {
                ...
                mcstep ( stx , fx , dgx , sty , fy , dgy , stp , f[0] , dg , brackt , stmin , stmax , infoc );
            }
