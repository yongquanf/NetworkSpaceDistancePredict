...
public class M3fshc_bias_Landmark<T> implements MFWithGradient {
	 ...
	 * @param dim2
	 * @param level2
	 * @param host2landmark2
	 * @param landmark2host2
	 * @param lambda2
	 */
	public M3fshc_bias_Landmark(int dim2, int level2, Matrix host2landmark2,
			Matrix landmark2host2, double lambda2) {
		 this.dim=dim2;
		 this.level=level2;
		 this.D_host2landmark=host2landmark2.copyMatrix();
		 this.D_landmark2host=landmark2host2.copyMatrix();
		 this.lambda=lambda2;
	...
	*/
	public double evaluate(double[] argument, double[] gradient) {
		...
		Matrix Bias=v.reshape(n*p+n*p+n*(l-1),n, 1);
		
		//bias matrix
		Matrix TotalBias=new DenseMatrix(n,n,0);
		for(int i=0;i<n;i++){
			for(int j=(i+1);j<n;j++){
				TotalBias.setValue(i, j, Bias.value(i, 0)
						+ Bias.value(j, 0));
				TotalBias.setValue(j,i,TotalBias.value(i, j));
			}
		}
		
		Matrix X=(U.multiply(V)).add(TotalBias);
		...
		Matrix dBias=Bias.scale(lambda);
		...
		for(int i=0;i<n;i++){
			...
			Bias.rowVector(i).dotProduct(Bias.rowVector(i));
		}
		val=(val*lambda)/2;		
		regobj+=val;
	...
	for(int k=1;k<=l-1;k++){
		...
		Vector out=new DenseVector(len7+len8+len9+len10);
		...
		for(int i=0;i<out.numDimensions();i++){
			if(i<len7){
				...
				out.setValue(i, tmp10.value(i-(len7+len8+len9)));
			}
		}
		tmp7.clear();
		tmp8.clear();
