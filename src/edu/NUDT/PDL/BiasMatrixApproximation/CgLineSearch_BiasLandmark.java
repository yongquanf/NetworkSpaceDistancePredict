...
/**
 * Line search, for bias based MMMF
 * @author Administrator
 *
 */
public class CgLineSearch_BiasLandmark {


			
			int verbose=1;
			
			static CgLineSearch_BiasLandmark self=null;
			public static  CgLineSearch_BiasLandmark getInstance(){
				if(self==null){
					self=new  CgLineSearch_BiasLandmark();
				}
				return self;
			}
			 ...
			 *change the gradient computation function
			 * @param alpha00 
			 */
			public void compute(Vector x0,double[] obj2,
					Vector dx0, Vector direction, M3fshc_bias_Landmark ogfun,			
		...
		(Conjgrad_gradient.pround(obj[0],digits) <= Conjgrad_gradient.pround(obj2[0],digits)) &&
			...
			private void backtrack(Vector x,double[] obj,
					Vector dx,Vector direction, M3fshc_bias_Landmark  ogfun,
